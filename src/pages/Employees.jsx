...
export const Employees = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEmployee) {
        const { error } = await supabase.from('employees').update({ name: formData.name }).eq('id', editingEmployee.id)
        if (error) throw error
      } else {
        const { error } = await supabase.from('employees').insert([{ name: formData.name }])
        if (error) throw error
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        {employees.length === 0 ? (
          <EmptyState
            title="Nuk ka punonjës ende"
            description="Shto punonjësit për t'i caktuar në porosi"
            action={
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {employees.map((emp) => (
              <div
                key={emp.id}
                className="p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-primary-400 transition-all group"
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium text-dark-500">{emp.name}</span>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleOpenModal(emp)} className="p-1 hover:bg-gray-200 rounded">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 justify-end pt-4">
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? 'Duke ruajtur...' : editingEmployee ? 'Përditëso' : 'Shto'}
            </Button>
          </div>
        </form>
      </Modal>
