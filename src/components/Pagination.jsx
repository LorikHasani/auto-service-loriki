  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100 mt-2">
      <p className="text-xs sm:text-sm text-gray-500">
        <span className="font-medium text-gray-700">{from}</span>–<span className="font-medium text-gray-700">{to}</span> nga <span className="font-medium text-gray-700">{totalItems}</span>
      </p>
          ...
          page === '...' ? (
            ...
            <button key={page} onClick={() => onPageChange(page)}
              className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${
                page === currentPage ? 'bg-primary-400 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
              }`}>
              {page}
            </button>
          )
